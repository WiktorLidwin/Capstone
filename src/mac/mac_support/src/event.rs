use crate::linux::peripherals::*;
use crate::windows::mouse::MouseEvent;
use crate::KeyboardEvent;

// fn from_windows_mouse_event(event: MouseEvent) -> LinuxEvent{

// }



fn from_windows_keyboard_event(event: KeyboardEvent) -> LinuxEvent{
    //if scan code is the same we chillin :D
    let code = translate_windows_code_to_linux(event.vkCode)

    LinuxEvent{type_: evdev_rs::enums::EventType as u32 as u16, code: code, value: (1 + (event.flags >> 7)) % 2 }
}
...
fn translate_windows_code_to_linux(code: u32) -> u32{
    match code{
        0 => 0,
        3 => 223,
        8 => 14,
